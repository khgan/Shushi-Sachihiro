...
$mysqli_db = new mysqli('localhost', 'root', '', 'shushi_sachihiro');
   ...
   if (!empty($name) && !empty($tel) && !empty($email) && !empty($description))
   {
        $query = 'INSERT INTO feedback(name, tel, email, description) 
			       VALUES(?,?,?,?)';

   $requery = $mysqli_db->prepare($query);
   $requery->bind_param("ssss", $name, $tel, $email, $description);
   
    if ($requery->execute()) {
         header('location:thankyou.php?id=' . $requery->insert_id);
      }
$mysqli_db->close();
   } 
    else {
      $msg = 'Enter your feeback!<br/>';
   }
}
?>
...
<ul class="contact-banner1">        
    ...
    <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" onsubmit="myFunction()">
     ...
     <label>Tel/Phone:<br>
     <input type = "tel" name = "phone" placeholder = "Please enter your tel" required = "on" style= "padding-bottom:20px ;padding-right:60px"/>
     </label><br><br>
     ...
     <label>Message:<br>
     <input type = "text" name = "description" placeholder = "Please enter the Message" 
     required = "on" style= "padding-bottom:120px ;padding-right:300px"/></label><br>
        
<input type="submit" id="buttonreserve" name="feedback" value="Submit">
</form>
</ul>
 ...
 <ul class="followus"><h1><br>GET INVITED</h1><br><p>Be the first to receive exclusive invitations to get Experience events, preview authentic <br>Japanese recipes by our top head chefs</p><br>
     ...
     required = "on" style= "padding-bottom:20px ;padding-right:60px"/></label><br>
         <input type="submit" id="buttonreserve" name="feedback" value="Subscribe Now">
     </form>
